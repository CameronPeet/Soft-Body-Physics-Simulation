...
Cloth::Cloth(std::vector<glm::vec3> &points, std::vector<GLuint> &indices)
{
...
@param program : The coupled vertex and fragment shader desired for the rendering of the calling instance
@param camera :  Address to the main camera, providing the view and projection matrices for rendering the view of the model.
void Cloth::Render(GLuint program, Camera& camera, std::vector<glm::vec3> &points, std::vector<GLuint> &indices, Light* lightSource)
{
glUseProgram(program);
...
glBindBuffer(GL_ARRAY_BUFFER, vbo);
glBufferSubData(GL_ARRAY_BUFFER, 0, 3 * sizeof(GLfloat) * vertices.size(), &vertices[0]);

glBindBuffer(GL_ARRAY_BUFFER, ebo);
glBufferSubData(GL_ARRAY_BUFFER, 0, 3 * sizeof(GLfloat) * indices.size(), &indices[0]);
